...
(rather than fixed-step) physics. It is the Isaac stack plus that integration — not a standalone
platform.

*Rubato* (It., "stolen") — the flexible variation of local tempo in music; here, adaptive timestepping,
where integration effort is reallocated across a frame (spent at stiff contact, withheld in free motion)
while the frame, and with it the fixed-rate control boundary, is preserved.
...
Measured on the Trossen cube-lift, the internal step spans ~4000:1 — microseconds at contact,
...
The adaptive solver can be selected three ways: the config flag `MJWarpSolverCfg(adaptive=True)`, the
env var `NEWTON_ADAPTIVE=1`, or the GUI toggle (the `newton_adaptive_ui` Kit extension). Drop
