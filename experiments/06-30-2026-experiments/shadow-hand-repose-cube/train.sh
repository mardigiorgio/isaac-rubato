...
TASK="${TASK:-Isaac-Reorient-Cube-Shadow-Rubato}"   # the gym id registered in this folder's env.py
GROUP="${GROUP:-shadow_hand_repose_cube}"               # W&B group: pairs THIS example's fixed+adaptive runs
# ================================================================
...
exec env "${ADAPTIVE_ENV[@]}" "$LAB_SH" train --rl_library rsl_rl \
  --task "$TASK" physics="${PHYSICS:-newton_mjwarp}" \
  --headless --seed "$SEED" \
